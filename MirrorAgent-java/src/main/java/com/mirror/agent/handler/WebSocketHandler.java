...
import com.mirror.agent.graph.*;
import com.mirror.agent.loader.*;
import com.mirror.agent.memory.*;
import com.mirror.agent.model.*;
import com.mirror.agent.rag.*;
...
public class WebSocketHandler extends TextWebSocketHandler {
    ...
    public WebSocketHandler(Orchestrator orchestrator, ChatAgent chatAgent,
                            ...
                            JDAnalyzer jdAnalyzer, ResumeMatcher resumeMatcher) {
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) {
        ...
        try {
            ...
            switch (msg.getType() != null ? msg.getType() : "") {
                ...
                case "match_check" -> handleMatchCheck(ws, msg);
    ...
    private void handleChat(WSSession ws, ClientMsg msg) {
        ...
        String userIntent = extractUserText(rawContent);
        String input = resolveInput(rawContent);

        // 优先级 1：已激活的 Skill
        if (ws.activeSkill != null) {
            if (SkillUtils.isQuitCommand(userIntent) || (ws.skillState != null && ws.skillState.isExpired())) {
                ws.activeSkill = null;
                ws.skillState = null;
            ...
            if (skillRegistry.match(userIntent) == null) {
        ...
        // 优先级 2：匹配新 Skill
        Skill matched = skillRegistry.match(userIntent);
        if (matched != null) {
        ...
        // 优先级 3：ChatAgent 流式兜底（逐 token 推送）
        try {
            chatAgent.stream(ws.chatHistory, input, userIntent, token ->
                    sendServerMsg(ws.conn, ServerMsg.builder().type("token").content(token).build()));
        } catch (Exception e) {
            log.warn("[WS] 流式生成异常: {}", e.getMessage());
            sendServerMsg(ws.conn, ServerMsg.builder().type("error").message("生成失败: " + e.getMessage()).build());
        }
        sendServerMsg(ws.conn, ServerMsg.builder().type("stream_end").build());
    }
    ...
    /** 提取用户自己的输入文本：忽略 [FILE:] 文件块，只取 \n---\n 之后的文字（无文件时返回原文） */
    private String extractUserText(String content) {
        if (content == null) return "";
        int sep = content.indexOf("\n---\n");
        if (sep >= 0) return content.substring(sep + 5).trim();
    ...
    private String resolveInput(String content) {
        ...
        // 处理 [FILE:filename]base64data 格式：一条消息可含多个连续文件块，
        // 末尾用 "\n---\n" 分隔用户补充文本（base64 为单行且不含 [ 与 ---）
        int sep = content.indexOf("\n---\n");
        String fileBlocks = sep >= 0 ? content.substring(0, sep) : content;
        String userText = sep >= 0 ? content.substring(sep + 5).trim() : "";

        if (fileBlocks.startsWith("[FILE:")) {
            StringBuilder resolved = new StringBuilder();
            String rest = fileBlocks;
            boolean hasFile = false;
            while (rest.startsWith("[FILE:")) {
                int endBracket = rest.indexOf(']');
                if (endBracket <= 6) break;
                String filename = rest.substring(6, endBracket);
                rest = rest.substring(endBracket + 1);
                int nextFile = rest.indexOf("[FILE:");
                String base64Data = nextFile >= 0 ? rest.substring(0, nextFile) : rest;
                rest = nextFile >= 0 ? rest.substring(nextFile) : "";
                try {
                    String text = documentLoader.parseBase64File(filename, base64Data.trim());
                    resolved.append("【文件 ").append(filename).append("】\n").append(text).append("\n\n");
                    hasFile = true;
                } catch (Exception e) {
            ...
            if (hasFile) {
                if (!userText.isEmpty()) resolved.append("【用户补充】\n").append(userText);
    ...
    /** 投递自检：JD × 简历 匹配报告（不进入面试） */
    private void handleMatchCheck(WSSession ws, ClientMsg msg) {
        String jdText = msg.getJd() != null ? msg.getJd() : "";
        String resumeText = msg.getResume() != null ? msg.getResume() : "";
        if (jdText.isEmpty() || resumeText.isEmpty()) {
            sendServerMsg(ws.conn, ServerMsg.builder().type("error").message("请同时提供意向 JD 与简历后再生成匹配报告").build());
            return;
        }
        asyncExecutor.submit(() -> {
            try {
                sendServerMsg(ws.conn, ServerMsg.builder().type("stage_change")
                        .stage("match_check").message("正在分析 JD 与简历，生成匹配报告...").build());
                JDAnalysis jd = jdAnalyzer.analyze(resolveInput(jdText));
                Resume resume = new Resume();
                resume.setRawText(resolveInput(resumeText));
                ResumeMatchResult r = resumeMatcher.match(jd, resume);
                sendServerMsg(ws.conn, ServerMsg.builder().type("match_report").content(buildMatchReport(jd, r)).build());
            } catch (Exception e) {
                log.warn("[WS] 匹配自检失败: {}", e.getMessage());
                sendServerMsg(ws.conn, ServerMsg.builder().type("error").message("匹配报告生成失败: " + e.getMessage()).build());
            }
        });
    }

    private String buildMatchReport(JDAnalysis jd, ResumeMatchResult r) {
        StringBuilder sb = new StringBuilder();
        double score = Math.max(0, Math.min(100, r.getOverallScore()));
        String grade = score >= 85 ? "A" : score >= 70 ? "B" : score >= 60 ? "C" : "D";
        sb.append("**匹配总分**：").append(String.format("%.0f", score)).append("/100（").append(grade).append("）\n");
        sb.append("**目标岗位**：").append(emptyOr(jd.getPosition(), "未知")).append("\n");
        sb.append("**经验要求**：").append(emptyOr(jd.getExperienceLevel(), "未标明")).append("\n\n");

        List<ResumeMatchResult.SkillMatch> skills =
                r.getSkillMatch() == null ? Collections.emptyList() : r.getSkillMatch();
        int reqTotal = 0, reqHit = 0, prefTotal = 0, prefHit = 0;
        for (ResumeMatchResult.SkillMatch s : skills) {
            boolean ok = s.isMatched() || s.getMatchScore() >= 60;
            if (s.isRequired()) { reqTotal++; if (ok) reqHit++; }
            else { prefTotal++; if (ok) prefHit++; }
        }
        int reqPct = reqTotal == 0 ? 100 : (int) Math.round(reqHit * 100.0 / reqTotal);
        int prefPct = prefTotal == 0 ? 0 : (int) Math.round(prefHit * 100.0 / prefTotal);

        sb.append("## 能力维度得分\n");
        sb.append("| 维度 | 得分 |\n|---|---|\n");
        sb.append("| 必备技能命中 | ").append(reqPct).append(" |\n");
        sb.append("| 加分技能命中 | ").append(prefPct).append(" |\n");
        sb.append("| 综合匹配 | ").append(String.format("%.0f", score)).append(" |\n\n");

        sb.append("## 技能逐条对照\n");
        if (skills.isEmpty()) {
            sb.append("- 未获得技能级对照数据\n");
        } else {
            for (ResumeMatchResult.SkillMatch s : skills) {
                String tag = (s.isMatched() || s.getMatchScore() >= 60) ? "[命中]" : s.getMatchScore() >= 30 ? "[部分]" : "[缺口]";
                String req = s.isRequired() ? "必备" : "加分";
                sb.append("- ").append(tag).append(" **").append(emptyOr(s.getSkillName(), "未知技能")).append("**（").append(req).append("）");
                sb.append(String.format("%.0f", s.getMatchScore())).append("分");
                if (s.getEvidence() != null && !s.getEvidence().isEmpty()) sb.append(" — ").append(s.getEvidence());
                sb.append("\n");
            }
        }
        appendList(sb, "优势", r.getStrengths(), 4);
        appendList(sb, "风险与不足", r.getWeaknesses(), 4);

        List<String> gaps = new ArrayList<>();
        for (ResumeMatchResult.SkillMatch s : skills) {
            if (s.isRequired() && !s.isMatched() && s.getMatchScore() < 60) gaps.add(s.getSkillName());
        }
        sb.append("## 建议\n");
        if (!gaps.isEmpty()) sb.append("- 优先补齐必备项：").append(String.join("、", gaps)).append("\n");
        sb.append("- 面试重点准备：");
        if (r.getFocusAreas() != null && !r.getFocusAreas().isEmpty()) {
            sb.append(String.join("；", r.getFocusAreas().subList(0, Math.min(4, r.getFocusAreas().size()))));
        } else {
            sb.append("围绕 JD 必备技能深挖");
        }
        sb.append("\n");
        return sb.toString();
    }

    private void appendList(StringBuilder sb, String title, List<String> items, int max) {
        sb.append("## ").append(title).append("\n");
        if (items == null || items.isEmpty()) { sb.append("- 无\n"); return; }
        int n = Math.min(max, items.size());
        for (int i = 0; i < n; i++) sb.append("- ").append(items.get(i)).append("\n");
    }

    private static String emptyOr(String s, String dflt) {
