...
public class LongTermMemory {
    ...
    /**
     * 更新技能等级（取历史最好水平，避免一次失误降级）
     * 分数 >= 80 精通 / >= 60 熟悉 / 其他 了解
     */
    public void updateSkillLevel(String userID, String skill, double score) {
        if (skill == null || skill.trim().isEmpty()) return;
        UserProfile profile = getProfile(userID);
        synchronized (profile) {
            Map<String, String> levels = profile.getSkillLevel();
            if (levels == null) {
                levels = new HashMap<>();
                profile.setSkillLevel(levels);
            }
            String level = score >= 80 ? "advanced" : score >= 60 ? "intermediate" : "beginner";
            String cur = levels.get(skill);
            int rank = "advanced".equals(level) ? 3 : "intermediate".equals(level) ? 2 : 1;
            int curRank = "advanced".equals(cur) ? 3 : "intermediate".equals(cur) ? 2 : "beginner".equals(cur) ? 1 : 0;
            if (rank > curRank) levels.put(skill, level);
            profile.setUpdatedAt(LocalDateTime.now());
        }
        if (store != null) store.saveProfile(profile);
    }
