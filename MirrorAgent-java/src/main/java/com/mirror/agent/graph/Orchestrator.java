            ...
            if (q.getSkills() != null) {
                try {
                    for (String skill : q.getSkills()) {
                        ...
                        longTermMem.updateSkillLevel(c.userID, skill, score.getScore());
                        if (mysqlStore != null) {
                            try {
                                mysqlStore.syncReviewItem(c.userID, skill, score.getScore());
                            } catch (Exception se) {
                                log.warn("[Orchestrator] 同步复习项失败（不影响主流程）: {}", se.getMessage());
                            }
                        }
                    }
                } catch (Exception e) {
        ...
        if (mysqlStore != null) {
            try {
                ...
                mysqlStore.saveInterviewRecord(c.userID, UserProfile.InterviewRecord.builder()
                        ...
                        .overallScore(c.report.getOverallScore())
                        .date(LocalDateTime.now())
                        .build(), reportJSON, planJSON, reportMD, planMD);
            } catch (Exception e) {
                log.warn("[Orchestrator] 保存面试记录到 MySQL 失败: {}", e.getMessage());
            }
        }

        // ===== 画像快照：记录本场结束后的薄弱点状态，用于"越练越懂"趋势曲线 =====
        if (mysqlStore != null) {
            try {
                java.util.List<UserProfile.WeakPoint> weak = longTermMem.getWeakPoints(c.userID);
                int weakCount = weak == null ? 0 : weak.size();
                double avgWeak = 0;
                if (weak != null && !weak.isEmpty()) {
                    double s = 0;
                    for (UserProfile.WeakPoint w : weak) {
                        s += w.getScore();
                    }
                    avgWeak = s / weak.size();
                }
                String weakJson = objectMapper.writeValueAsString(weak);
                mysqlStore.saveProfileSnapshot(c.userID, c.session.getId(),
                        weakCount, Math.round(avgWeak * 10) / 10.0, weakJson);
            } catch (Exception e) {
                log.warn("[Orchestrator] 保存画像快照失败: {}", e.getMessage());
            }
        }
